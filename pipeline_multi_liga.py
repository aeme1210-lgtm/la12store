...
def importar_liga(league_name, league_folder):
    ...
    # Borrar productos existentes de esta liga
    cur.execute('DELETE FROM "Product" WHERE league = %s', (league_name,))
    deleted = cur.rowcount
    conn.commit()
    log(f"    Eliminados {deleted} productos anteriores de {league_name}")

    # Cargar slugs usados (después del delete para evitar colisiones)
    ...
    cur.close()
    conn.close()
    log(f"    Importación completa: {created} productos, {errors} errores")
    return created
