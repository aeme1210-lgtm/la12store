...
# entre los 5 pipelines). El parámetro `mode` ya no aplica: upsert_product
# siempre detecta el producto existente por identidad normalizada y lo
# actualiza en vez de duplicarlo o borrar la liga completa.
...
def import_league(league_name, folder_name, mode="replace"):
    ...
    for fd in folders:
        ...
        try:
            result = upsert_product(cur, league_name, fd.name, imgs)
            if result == "created":
                created += 1
            else:
                updated += 1
